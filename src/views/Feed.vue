          ...
          <template v-else>
            ...
            <template v-if="assemblyPanel(photo.id).details.length">
              ...
              <div class="assembly-details">
                <div v-for="item in assemblyPanel(photo.id).details" :key="item.assemblyDetailId || item.id" class="assembly-detail">
                  <strong>{{ item.detail?.name || item.detailInfo?.nameDetail || item.name || 'Деталь' }}</strong>
                  <span v-if="item.detail?.type?.name || item.detailInfo?.nameType" class="detail-type">{{ item.detail?.type?.name || item.detailInfo?.nameType }}</span>
...
const assemblyLikeState=photo=>{const assembly=photo.assembly||{};const id=assembly.id;if(!assemblyLikes[id]){const rawCount=assembly.likesCount;assemblyLikes[id]={liked:Boolean(assembly.likedByMe),likesCount:Number(rawCount)||0,hasCount:rawCount!==null&&rawCount!==undefined&&Number.isFinite(Number(rawCount)),initialized:assembly.likedByMe!==undefined,busy:false}}return assemblyLikes[id]};
...
const loadAssembly=async(photo,reload=false)=>{const panel=assemblyPanel(photo.id);if(panel.loading)return;panel.loading=true;panel.error='';try{const response=await feedApi.getPublicAssembly(photo.assembly.id);const info=response?.assembly||response;panel.info=info||photo.assembly;panel.details=info?.components||info?.assemblyDetails||info?.details||[];const state=assemblyLikeState(photo);if(info?.likedByMe!==undefined)state.liked=Boolean(info.likedByMe);if(info?.likesCount!==undefined){state.likesCount=Number(info.likesCount)||0;state.hasCount=true}state.initialized=true;panel.loaded=true}catch(e){panel.error=getApiErrorMessage(e,'Не удалось загрузить публичную сборку');if(reload)panel.loaded=false}finally{panel.loading=false}};
...
const toggleAssemblyLike=async photo=>{const id=photo.assembly?.id;if(!id)return;const current=assemblyLikeState(photo);if(current.busy)return;current.busy=true;try{if(!current.initialized){const response=await feedApi.getPublicAssembly(id);const info=response?.assembly||response;current.liked=Boolean(info?.likedByMe);if(info?.likesCount!==undefined){current.likesCount=Number(info.likesCount)||0;current.hasCount=true}current.initialized=true}const state=current.liked?await feedApi.unlikeAssembly(id):await feedApi.likeAssembly(id);current.liked=Boolean(state.liked);current.likesCount=Number(state.likesCount)||0;current.hasCount=true}catch(e){notify(e,'Не удалось изменить лайк сборки')}finally{current.busy=false}};
